...
export const CodeRenderer = {
  code(code: string, infostring: string | undefined, escaped: boolean) {
    ...
    switch (infostring.toLowerCase()) {
      case "mysql":
        return `<pre class="language-mysql pre-${themeColor}"><code class="language-mysql">${
      ...
      default:
        return `<pre class="language-${infostring} pre-${themeColor}"><code class="language-${infostring}">${
          hljs.highlight(code, {
            language: infostring,
